...
- The `backup-4-segmented` default feed library is forced once onto existing installs that do not have that default-library version marker, so old saved default feeds do not keep winning over the shipped backup defaults.
