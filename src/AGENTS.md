...
- Built-in sensitive feed segments are installed once without replacing saved feeds. Their Feeds-page visibility follows the existing BL/GL and Smut/Hentai search toggles, and they remain excluded from Home by default. Opening a hidden segment feed previews only that segment in Home until the Home shortcut clears it. Newly created feeds always enter UNSEGMENTED, never a built-in segment.
